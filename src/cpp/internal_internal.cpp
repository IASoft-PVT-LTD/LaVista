...
window.addEventListener("resize",lavistaSyncMaxIcon);
    ...
    static auto read_entire_file_binary(const filesystem::Path &path) -> Result<Vec<u8>>
    {
      ...
      if (!f)
      {
        return fail("Failed to open icon file for title bar: {}", path.string());
      }
      f.seekg(0, std::ios::end);
      const auto sz = f.tellg();
      if (sz <= 0)
      {
        return fail("Icon file for title bar is empty: {}", path.string());
      }
      ...
      if (!f)
      {
        return fail("Failed to read icon file for title bar: {}", path.string());
      }
      return out;
    }
  ...
  auto load_inline_html_into_webview(Window window, webview_t w, String html_document) -> Result<void>
  {
    ...
    temp_html /=
        (String("lavista-html-") + String::format("{}", static_cast<unsigned long long>(tick)) + ".html").c_str();
    {
      std::ofstream out(temp_html, std::ios::binary | std::ios::trunc);
      if (!out)
      {
        return fail("Cannot write temporary HTML: {}", temp_html.string());
      }
    ...
    if (window != nullptr)
    {
      window_ptr(window)->temp_files.push_back(temp_html);
    }
...
namespace LaVista::utils
{
                                   ...
                                   const filesystem::Path &bundle_dir_abs) -> Result<void>
  {
    ...
    const u64 tick = static_cast<u64>(std::chrono::steady_clock::now().time_since_epoch().count());
    temp_html /=
        (String("lavista-spa-") + String::format("{}", static_cast<unsigned long long>(tick)) + ".html").c_str();
    {
      std::ofstream out(temp_html, std::ios::binary | std::ios::trunc);
      if (!out)
      {
        return fail("Cannot write temporary SPA HTML: {}", temp_html.string());
      }
  ...
  auto load_image_rgba_from_file(const String &path, Vec<u8> &out_rgba, i32 &out_w, i32 &out_h) -> Result<void>
  {
    ...
    if (data == nullptr)
    {
      ...
      return fail("Failed to decode window icon '{}': {}", p, why != nullptr ? why : "unknown error");
    }
    if (w <= 0 || h <= 0)
    {
